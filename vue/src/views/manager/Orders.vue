      ...
      <el-table :data="tableData" stripe>
        <el-table-column prop="name" label="商品名称" :show-overflow-tooltip="true" width="200">
          <template v-slot="scope">
            <el-link :href="'/front/goodsDetail/' + scope.row.goodsId" :underline="false">{{ scope.row.name }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="time" label="购买时间" :show-overflow-tooltip="true">
          <template v-slot="scope">
        ...
        <el-table-column label="操作" align="center" width="240">
          <template v-slot="scope">
            <el-button size="mini" type="success" plain @click="ship(scope.row)" v-if="scope.row.state === '未发货'">发货</el-button>
            <el-button size="mini" type="info" plain @click="detail(scope.row)">详情</el-button>
      ...
      <div class="drawer-content" ref="drawerContent">
        <el-form label-width="100px" style="padding-right: 40px" :model="form">
          ...
          <el-form-item label="商品封面" >
            <div>
              <el-image v-if="form.goods && form.goods.cover" style="width: 50px; height: 50px" :src="form.goods.cover" fit="cover" :preview-src-list="[form.goods.cover]"></el-image>
            </div>
          </el-form-item>
          ...
          <el-form-item label="购买时间" prop="name">
            <div>{{ form.time || '暂无' }}</div>
          </el-form-item>
          <el-form-item label="订单状态" prop="name">
            <div>{{ form.state }}</div>
          </el-form-item>
          <el-form-item label="用户" >
            <div>{{ form.user && form.user.name }}</div>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    ship(row) {
      this.$confirm('确认发货吗？', '确认发货', {type: 'warning'}).then(() => {
        row.state = '已发货'
        this.$request.put('/orders/update', row).then(res => {
          ...
          this.$notify.error({title: '错误', message: '发货失败，请稍后重试', showClose: false, duration: 2000})
        })
      }).catch(() => {})
    },
    del(id) {
