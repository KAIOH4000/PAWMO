      ...
      <el-table :data="tableData" stripe>
        ...
        <el-table-column align="center" prop="cover" label="图片">
          <template v-slot="scope">
            <el-image style="width: 50px; height: 50px" :src="getImageUrl(scope.row.cover)" :preview-src-list="[getImageUrl(scope.row.cover)]"></el-image>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    getImageUrl(cover) {
      ...
      return 'http://localhost:9999' + cover;
    },
    setEditor() {
      this.$nextTick(() => {
        // 关键：创建新实例前，先销毁已存在的实例
        ...
        if (!editorElem) {
          console.error('编辑器容器 #editor 不存在');
          return;
        }
        // 重新创建编辑器实例
        ...
        // 配置 z-index 确保编辑器显示在弹窗之上
        this.editor.config.zIndex = 500;
        try {
          this.editor.create(); // 最终创建实例
        } catch (e) {
          console.error('wangEditor 创建失败:', e);
        }
      });
    },
    destroy() {
